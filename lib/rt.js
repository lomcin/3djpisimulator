...
CRuntime.prototype.defFunc = function(lt, name, retType, argTypes, argNames, stmts, interp, optionalArgs) {
  ...
  rt = this;
  if (stmts != null) {
    f = function*(rt, _this, ...args) {
      var argValue, i, j, optionalArg, ref, ret;
        ...
        Object.keys(reg).forEach((signature) => {
          ...
          if (dts.length <= newTs.length) {
            ...
            while (ok && i < dts.length) {
              ok = this.castable(newTs[i], dts[i]);
              i++;
            }
            while (ok && i < newTs.length) {
              ok = this.castable(newTs[i], optionalArgs[i - dts.length].type);
              i++;
            }
            if (ok) {
              compatibles.push(t[name][this.makeParametersSignature(regArgInfo.args)]);
            }
          }
        });
    ...
    if (type.name === "bool") {
      ...
      if (type.name.slice(0, 8) === "unsigned") {
        if (!this.isNumericType(value.t)) {
          ...
          ({bytes} = this.config.limits[type.name]);
  ...
  if (Array.isArray(type)) {
    if (type.length > 1) {
      typeStr = type.filter((t) => {
        return indexOf.call(this.config.specifiers, t) < 0;
      }).join(" ");
...
CRuntime.prototype.makeTypeString = function(type) {
  ...
  if (type.type === "primitive") {
    ...
    if (type.ptrType === "normal") {
      ...
      ret += this.makeTypeString(type.retType) + "(*f)" + "(" + type.signature.map((e) => {

...
Interpreter = function(rt) {
  ...
  this.visitors = {
    ...
    TypedefDeclaration: function*(interp, s, param) {
      var _basetype, basetype, declarator, k, len, name, ref, type;
      rt = interp.rt;
      ...
      for (k = 0, len = ref.length; k < len; k++) {
        declarator = ref[k];
        ({type, name} = (yield* interp.visit(interp, declarator, param)));
    ...
    Declaration: function*(interp, s, param) {
      var _basetype, basetype, dec, dim, dimensions, i, init, initializer, j, k, l, len, len1, name, ref, ref1, type;
      rt = interp.rt;
      basetype = rt.simpleType(s.DeclarationSpecifiers);
      ...
      for (i = k = 0, len = ref.length; k < len; i = ++k) {
        ...
        if (dec.Declarator.right.length > 0 && dec.Declarator.right[0].type === "DirectDeclarator_modifier_array") {
          ...
          _basetype = param.basetype;
          param.basetype = basetype;
          ({name, type} = (yield* interp.visit(interp, dec.Declarator, param)));
          param.basetype = _basetype;
